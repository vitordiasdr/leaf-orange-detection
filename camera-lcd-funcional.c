...
#endif
// picture buffer
//__attribute__((section(".camera_buffer")))
//__attribute__((aligned(32)))
//uint16_t camera_fb[320 * 240];
#define FRAME_PIXELS (FrameWidth * FrameHeight)

/* Buffer onde o DCMI + DMA escrevem */
__attribute__((section(".RAM_D2"))) uint16_t frame_dma[FRAME_PIXELS];

/* Buffer exclusivo para o LCD */
__attribute__((section(".RAM_D2"))) uint16_t frame_lcd[FRAME_PIXELS];
...
static void MPU_Config(void)
{
  ...
  /* Region 0: DTCM RAM (Stack, Data, BSS) */
  ...
  /* Region 1: RAM_D2 – DMA / Framebuffer (NON cacheable) */
  MPU_InitStruct.Enable           = MPU_REGION_ENABLE;
...
while (1)
{
    ...
    if (frame_ready)
    {
        ...
        /* Copia frame completo e estável */
        memcpy(frame_lcd, frame_dma, FRAME_PIXELS * 2);

        /* Atualiza LCD com UM frame coerente */
        ST7735_FillRGBRect(
            &st7735_pObj,
            0, 0,
            (uint8_t *)frame_lcd,
...
void HAL_DCMI_FrameEventCallback(DCMI_HandleTypeDef *hdcmi)
{
    ...
    {
        tick = HAL_GetTick();
        Camera_FPS = frames;  // seu print de FPS
        frames = 0;
    }

    frame_ready = 1;  // SÓ ISSO
}
/* USER CODE END 4 */
